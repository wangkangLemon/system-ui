...
class OrderService {
    // 订单记录列表
    search({page, page_size, status, channel = '', product_name = '', product_category = '', product_order_no = ''}) {
        let url = `${urlPre}/search`
        return api.get(url, {page, page_size, status, channel, product_name, product_category, product_order_no}).then((ret) => {
            return ret.data
        })
    }
    // 查看物流信息
