...
<script>
    ...
    export default{
        ...
        methods: {
            init () {
                if (this.id) {
                    practiceService.view(this.id).then((ret) => {
            ...
            submit () {
                this.$refs['form'].validate((valid) => {
                    ...
                    if (this.id) {
                        practiceService.update(this.id, this.fetchParam).then((ret) => {
                            xmview.showTip('success', '操作成功')
                            this.submitLoading = false
                            this.$router.push({name: 'exam-practice-index'})
                        })
                    } else {
                        practiceService.create(this.fetchParam).then((ret) => {
                            xmview.showTip('success', '操作成功')
                            this.submitLoading = false
                            this.$router.push({name: 'exam-practice-index'})
                        })
            ...
            fetchSubjectGroup () {
                ...
                libraryService.search(params).then(ret => {
