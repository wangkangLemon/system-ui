...
<template>
    <div id="paper-content-container">
        ...
        <el-table class="data-table" v-loading="loadingData"
            ...
            <el-table-column label="错题类型" width="95">
...
<script>
    import practiceStatService from 'services/exam/practiceStatService'
    ...
    export default {
        ...
        methods: {
            ...
            fetchData () {
                ...
                practiceStatService.searchSubject(fetchParam).then((ret) => {
                    this.data = ret.list
            ...
            exportFn () {
                let fetchParam = _.clone(this.fetchParam)
                return practiceStatService.exportSubject(fetchParam).then(() => {
                    xmview.showTip('success', '任务创建成功，请到【报表中心】查看任务进度')
                })
            },
            fetchLibrary (keyword, length) {
                let page = parseInt(length / 10) + 1
                return libraryService.search({keyword, page}).then((ret) => {
                    let result = {
                        data: [],
                        total: ret.total
                    }
