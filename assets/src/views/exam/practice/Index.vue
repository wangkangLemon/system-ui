    ...
    export default {
        ...
        methods: {
            ...
            fetchData () {
                ...
                practiceService.search(fetchParam).then((ret) => {
                    this.data = ret.list
            ...
            del (index, row) {
                this.$confirm('您是否确定删除练习？', '删除', {
                    ...
                    return practiceService.delete(row.id).then((ret) => {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        })
            ...
            delMulti () {
                xmview.showDialog(`你将要删除选中的练习，操作不可恢复确认吗?`, () => {
                    practiceService.batchDelete(this.selectedIds).then(() => {
                        this.selectedIds = []
            ...
            online (index, row) {
                return practiceService.online(row.id).then(() => {
                    row.status = 0
                    row.start_time = ''
                    xmview.showTip('success', '操作成功')
                })
            ...
            offline (index, row) {
                return practiceService.offline(row.id).then(() => {
                    row.status = 1
                    row.start_time = ''
                    xmview.showTip('success', '操作成功')
                })
