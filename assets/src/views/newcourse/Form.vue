  ...
  <article id="add-newcourse">
    <el-tabs type="card" v-model="activeTab" class="tab">
      <el-tab-pane label="课程信息" name="couse">
        <el-form label-width="120px" ref="form" :rules="rules" :model="fetchParam">
          ...
          <el-form-item label="课程封面图" prop="image">
            <img
              :src="fetchParam.image | fillImgPath"
              width="200"
              height="112"
          ...
          <el-form-item label="课程介绍" prop="description">
            <el-input
              v-model="fetchParam.description"
              type="textarea"
          ...
          <el-form-item label="更新时间" prop="update_time_name" v-if="fetchParam.update_time_name">
            <el-date-picker
              v-model="fetchParam.update_time_name"
              type="datetime"
      ...
      <el-tab-pane :disabled="!fetchParam.id" label="课时管理" name="classhour">
        <!--单节课-->
        <el-form
          v-if="fetchParam.lesson_type == 'single'"
          label-width="120px"
          ref="classhourform"
          ...
          <el-form-item v-if="classhour.paperLesson">
            <section>
              ...
              <span class="operate" style="float: right">
                <el-button
                  type="text"
                  @click="openEditExamForm(classhour.paperLesson, -1, 0, $event)"
                >编辑</el-button>
        ...
        <section class="mulit-class" v-if="fetchParam.lesson_type == 'multi'">
          <div v-for="(item,index) in multi.data" :key="item.id">
            <div v-if="!item.deleted">
              <div v-if="item.id === -1">
                <p>
                  <el-button type="gray" size="full" @click="addNewClasshour">
                    <i class="el-icon-plus"></i> 添加新课时
                  </el-button>
                </p>
                <p>
                  <el-button
        ...
        <section class="mulit-class" v-if="fetchParam.lesson_type == 'chapter'">
            ...
            <section v-if="pitem.deleted == false">
              ...
              <div v-for="(item,index) in pitem.lessons" :key="item.id">
                <div v-if="!item.deleted">
                  <div v-if="item.id === -1">
                    <p>
                      <el-button type="gray" size="full" @click="addNewClasshour(pindex)">
                        <i class="el-icon-plus"></i> 添加新课时
                      </el-button>
                    </p>
                    <p>
                  ...
                  <p v-else>
                    ...
                    <span class="operate" v-else>
                      <el-button
                        type="text"
                        @click="openEditExamForm(item, pindex, index, $event)"
                      >编辑</el-button>
                      <el-button type="text" @click="delClasshour(item, pindex, index)">删除</el-button>
                    </span>
                  </p>
          ...
          <p v-show="!chapter.editStatus">
            <el-button
              type="gray"
              size="full"
              @click="()=>{chapter.editStatus = true;chapter.value = '';}"
            >
              <i class="el-icon-plus"></i> 添加新章节
            </el-button>
          </p>
          <el-button type="primary" class="saveBtn" @click="saveResult">保存</el-button>
        </section>
    ...
    <el-dialog :title="classhour.title" :visible.sync="classhour.showDialog" append-to-body>
    ...
    <Sidebar ref="paperSidebar" v-if="this.$route.query.course_type=='public'">
    ...
    if (this.$route.params.course_id != undefined) {
      newcourseService
        ...
        .then(ret => {
          ...
          this.courseTags =
            ret.course.tags === "" ? [] : ret.course.tags.split(",");
  ...
  methods: {
    ...
    addMultiSubmit() {
      this.$refs["multiForm"].validate(valid => {
        ...
        if (this.fetchParam.lesson_type === "multi") {
          this.currentData.pindex !== -1
            ? (this.multi.data[this.currentData.pindex] = item)
            : this.multi.data.splice(this.multi.data.length - 1, 0, item);
        } else if (this.fetchParam.lesson_type === "chapter") {
          this.currentData.index !== -1
            ? (this.resultData[this.currentData.pindex].lessons[
                this.currentData.index
              ] = item)
            : this.resultData[this.currentData.pindex].lessons.splice(
                this.resultData[this.currentData.pindex].lessons.length - 1,
                0,
                item
              );
        }
    ...
    submitPaper() {
      this.editPaper.save().then(() => {
        ...
        if (this.fetchParam.lesson_type === "multi") {
          this.currentData.index !== -1
            ? (this.multi.data[this.currentData.index] = lesson)
            : this.multi.data.splice(this.multi.data.length - 1, 0, lesson);
        }
