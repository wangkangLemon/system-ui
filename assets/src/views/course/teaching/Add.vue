  ...
  <main id="teaching-map-template-container">
    ...
    >
      <el-form-item label="模板名称" prop="title">
        <el-input :disabled="view" class="custom-input" v-model="ruleForm.title"></el-input>
      </el-form-item>
      <el-form-item label="分类" prop="category_id" >
        <el-select v-model="ruleForm.category_id" placeholder="请选择" filterable clearable>
          <el-option
      ...
      <el-form-item label="结业考试">
        ...
        <div class="task-item" v-if="ruleForm.finish_exam_name">
          ...
          <i class="el-icon-delete delete" title="删除任务" v-if="!view" @click="deleteFinishExam"></i>
        </div>
      </el-form-item>
      ...
      <el-form-item>
        ...
        >{{ruleForm.data ? '更新' : '发布'}}</el-button>
        <el-button type="primary" @click="$router.replace({name: 'course-teaching'})">关闭</el-button>
      </el-form-item>
    </el-form>
    <!-- 添加考试 -->
    <Exam title="选取考试" :visible.sync="showExamDialog" @pick="getFinishedExamData"></Exam>
  </main>
</template>
...
export default {
  ...
  methods: {
    fetchData() {
        return mapTypeService.getMapTypeList({page:'',page_size:'99999999',name:''}).then(ret => {
    ...
    submitForm(formName) {
      if (this.validateAllForm(formName)) {
        ...
        let service = this.ruleForm.data
          ? mapService.updateMap
          : mapService.createMap;
        service(this.ruleForm.save())
          .then(() => {
    ...
    validateAllForm(formName) {
      return ![...this.$refs.phase.map(item => item), this].some(
        (item, index) => !item.validate(formName, index + 1)
      );
    },
    validate(formName) {
      let pass = true;
