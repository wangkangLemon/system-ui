...
<template>
    <article id="course-manage-coursecategory">
        ...
        <section class="left-container">
            <SectionCategoryTree v-model="treeData" ref="sectionCategory"
                                :onNodeClick="treeNodeClick.bind(this,1)"></SectionCategoryTree>
        </section>
        ...
        <div class="el-dialog__wrapper" v-show="dialogTree.isShow">
            <article class="el-dialog el-dialog--tiny">
                ...
                <section class="el-dialog__body">
                    <SectionCategoryTree v-model="treeData" node-key="id"
                                        :onNodeClick="treeNodeClick.bind(this,2)"></SectionCategoryTree>
                </section>
...
<script type="text/jsx">
    ...
    import SectionCategoryTree from '../../component/tree/SectionCategory.vue'
        ...
        methods: {
            ...
            deleteCategory (){
                ...
                this.dialogConfirm.confirmClick = () => {
                    sectionService.delCategory({id: node.value}).then(() => {
                        xmview.showTip('success', '操作成功!')
                        this.$refs.sectionCategory.removeItem(node, this.nodeParentSelected)
                        node = null
            ...
            addRootCategory () {
                this.activeTab = 'add'
                // 清空选中项
                this.$refs.sectionCategory.clearSelected()
            ...
            submitForm () {
                this.$refs.form.validate((ret) => {
                    ...
                    if (this.activeTab === 'add') {
                        if (this.fetchParam.parent_id == undefined) {
                            xmview.showTip('warning', '请选择分类')
                            return false
                        }
                        p = sectionService.createCategory(this.fetchParam)
                    } else {
                        p = sectionService.updateCategory(this.fetchParam)
                    }
                    p.then((ret) => {
                        xmview.showTip('success', '操作成功!')
...
//                            this.fetchParam.id = ret.data.id
            ...
            moveSubCategory () {
                ...
                this.dialogTree.confirmClick = () => {
                    ...
                    sectionService.moveCategory({id, to}).then((ret) => {
                        // 重新渲染树节点
                        if (ret.code === 0) {
                            xmview.showTip('success', '操作成功!')
                            this.$refs.sectionCategory.initData()
            ...
            moveSubCategoryContent () {
                ...
                this.dialogTree.confirmClick = () => {
                    ...
                    sectionService.moveCategoryContent({id, to}).then((ret) => {
                        // 重新渲染树节点
                        if (ret.code === 0) {
                            xmview.showTip('success', '操作成功!')
                            this.dialogTree.isShow = false
