			...
			<el-form class="addForm" :model="form" :rules="rules" ref="form" :label-width="formLabelWidth">
				<el-form-item prop="category" label="企业类型">
					<el-select v-model="form.category">
						<el-option v-for="(item, index) in companyTypes" :label="item.name" :value="item.id" :key="item.id">
						</el-option>
					</el-select>
				</el-form-item>
				...
				<el-form-item label="地区" :label-width="formLabelWidth">
					<Region :province="form.province" :city="form.city" :area="form.area" title="" v-on:provinceChange="val => form.province = val" v-on:cityChange="val => form.city = val" v-on:areaChange="val => form.area = val">
					</Region>
				</el-form-item>
				...
				<div v-if="form.category===2">
					...
					<el-form-item prop="department_number" label="签约门店数量" :label-width="formLabelWidth">
						<el-input min="0" placeholder="签约门店数量" type="number" v-model="sign.department_number" auto-complete="off"></el-input>
					</el-form-item>
					<el-form-item prop="user_number" label="签约店员数量" :label-width="formLabelWidth">
						<el-input min="0" placeholder="签约店员数量" type="number" v-model="sign.user_number" auto-complete="off"></el-input>
					</el-form-item>
					...
					<el-form-item prop="sign_time" label="签约时间" :label-width="formLabelWidth">
						<el-date-picker :editable="false" v-model="sign.sign_time" type="date" :picker-options="pickerOptionsStart" placeholder="开始日期">
						</el-date-picker>
					</el-form-item>
					<el-form-item prop="expire_time" label="合同到期日" :label-width="formLabelWidth">
						<el-date-picker v-model="sign.expire_time" :editable="false" type="date" :picker-options="pickerOptionsEnd" placeholder="合同到期日">
						</el-date-picker>
					</el-form-item>
				</div>
				<div v-if="form.category==4">
					<div class="tip">
						* 以下信息关系营销中心数据, 核实签约数据后填写。
					</div>
					<el-form-item prop="department_count" label="关联门店数" :label-width="formLabelWidth">
						<el-input min="0" placeholder="关联门店数量" type="number" v-model="form.department_count" auto-complete="off"></el-input>
					</el-form-item>
				</div>
			</el-form>
...
<script>
	...
	export default {
		...
		created() {
			_this = this
			_this.companyID=_this.$route.query.id
		},
		activated() {
			_this = this
			_this.companyID=_this.$route.query.id
